...
export const getProducts = async () => {
  try {
    ...
    if (err.message?.includes("404")) {
      console.error("Đã xảy ra lỗi 404", err);
    } else {
...
export const getSuggestion = async (favorites: number[]): Promise<Product[]> => {
  ...
  const keywords: string[] = favoriteProducts
  ...
  if (suggestedProducts.length === 0) {
    ...
    suggestedProducts = allProducts
      .filter((p: Product) => !favorites.includes(p.id))
      .slice(0, 3);
  }
